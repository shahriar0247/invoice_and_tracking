...
function Create_Daily_Account({ create = true }) {
    ...
    async function get_purchase_order_details() {
        ...
        const response = await fetch('http://35.209.219.229:5003/get_daily_account_details/' + number);
    ...
    function fetchInvoice(invoice_id_) {
        fetch('http://35.209.219.229:5003/get_invoice_details_for_daily_accounts/' + invoice_id_)
            .then((response) => response.json())
            .then((data) => {
    ...
    function createDaily_Account() {
        ...
        fetch('http://35.209.219.229:5003/create/daily_account', {
            method: 'POST',
            headers: {
